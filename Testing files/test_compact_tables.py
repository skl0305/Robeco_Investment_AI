#!/usr/bin/env python3
"""
Test script to verify compact financial tables with actual Link REIT data
"""
...
def test_with_link_reit():
    """Test compact tables with actual Link REIT data"""
    print("🔍 Testing compact financial tables with Link REIT (823.HK) data...")
    
    # Fetch Link REIT data
    ticker = "823.HK"  # Link REIT
    ...
    try:
        ...
        if years and income_data:
            latest_year = years[0]
            print(f"\n💰 Sample Link REIT financial data for {latest_year}:")
            latest_income = income_data[latest_year]
            
            revenue = latest_income.get('Total Revenue')
            net_income = latest_income.get('Net Income')
            print(f"  - Total Revenue: {format_financial_value(revenue)}")
            print(f"  - Net Income: {format_financial_value(net_income)}")
            ...
            if balance_data and latest_year in balance_data:
                ...
                print(f"  - Total Assets: {format_financial_value(total_assets)}")
...
<html>
<head>
    <title>Link REIT Compact Financial Tables Test</title>
    <meta charset="utf-8">
</head>
<body style="margin: 20px; font-family: Arial, sans-serif;">
    <h1>🏢 Link REIT (823.HK) - Compact Financial Tables Test</h1>
...
"""
        ...
        # Save the test file
        output_file = "/Users/skl/Desktop/Robeco Reporting/src/robeco/backend/link_reit_compact_tables_test.html"
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(test_output)
        
        print(f"\n✅ Test complete! Results saved to: {output_file}")
        print(f"📝 Open the file in a browser to see the compact tables with actual Link REIT data")
